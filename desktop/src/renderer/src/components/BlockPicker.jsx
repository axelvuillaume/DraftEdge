...
export default function BlockPicker({ user, value, onChange }) {
  ...
  async function create(e) {
    ...
    setSaving(false)
    if (!res.ok) return setError(res.code || 'Création impossible')
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4 space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="font-medium">Bloc de scrim</h2>
        ...
        <select value={value?._id || ''} onChange={(e) => onChange(sessions.find((s) => s._id === e.target.value) || null)} className="w-full rounded-lg bg-slate-950 border border-slate-800 px-3 py-2 text-sm focus:outline-none focus:border-amber-500">
          ...
          {sessions.map((s) => (
            <option key={s._id} value={s._id}>
              {formatDate(s.date)} · {s.name || 'Sans nom'}
      ...
      {creating && (
        <form onSubmit={create} className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <input placeholder="Nom du bloc" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="rounded-lg bg-slate-950 border border-slate-800 px-3 py-2 text-sm focus:outline-none focus:border-amber-500" />
      ...
      {value && (
        <p className="text-xs text-slate-400">
          {value.opponent_name ? (
            <>
              Adversaire <span className="text-slate-200">{value.opponent_name}</span> ·{' '}
            </>
          ) : (
            <span className="text-amber-400">Pas d&apos;adversaire renseigné · </span>
          )}
