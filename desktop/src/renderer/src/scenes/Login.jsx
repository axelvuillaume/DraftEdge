...
export default function Login({ onLogin }) {
  ...
  async function submit(e) {
    ...
    setLoading(false)
    if (!res.ok) {
      if (res.code === 'UNAUTHORIZED' || res.status === 401) return setError('Invalid email or password')
      if (res.code === 'NETWORK_ERROR') return setError(`Server unreachable (${res.details}). Check your internet connection.`)
      return setError(`Login failed (${res.code || 'unknown error'})`)
    }
    onLogin(res.user)
  }
