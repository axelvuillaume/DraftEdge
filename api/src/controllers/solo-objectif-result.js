...
// Un aggregate compte des games (metric games_played) ou des wins (metric win) sur une période.
function countMatches(matches, metric) {
  if (metric === 'win') return matches.filter((m) => m.win).length;
  return matches.length;
}

function isHit(current, operator, value) {
...
const startOfWeek = (d) => {
  ...
  start.setDate(start.getDate() - (d.getDay() === 0 ? 6 : d.getDay() - 1)); // Lundi
  return start;
};
...
router.post('/aggregate', passport.authenticate(['admin', 'user'], { session: false, failWithError: true }), async (req, res) => {
  try {
    ...
    for (const obj of objectives) {
      const period = obj.aggregate?.period;
      ...
      if (period === 'weekly') periodStart = startOfWeek(now);

      const matchQuery = await buildMatchQuery(obj);
      if (periodStart) matchQuery.gameDate = { $gte: periodStart };
      const matches = await SoloqMatch.find(matchQuery, { win: 1 });
...
router.post('/aggregate-history', passport.authenticate(['admin', 'user'], { session: false, failWithError: true }), async (req, res) => {
  try {
    ...
    const period = obj.aggregate?.period;
    const { metric, operator, value } = obj.rule || {};
    if (!period || !metric) return res.status(200).send({ ok: true, data: [] });

    const now = new Date();
    const buckets = [];
    if (period === 'total') buckets.push({ start: new Date(obj.createdAt), end: now });
    if (period !== 'total') {
      const step = period === 'weekly' ? 7 : 1;
      let cursor = period === 'weekly' ? startOfWeek(new Date(obj.createdAt)) : startOfDay(new Date(obj.createdAt));
      while (cursor <= now) {
