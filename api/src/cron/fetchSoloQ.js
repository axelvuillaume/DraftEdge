...
function evaluateObjectives(objectives, doc, timeline, participantId, matchId, player) {
  ...
  for (const obj of objectives) {
    ...
    results.push({
      ...
      opponent_champion: doc.opponentChampion || null,
      win: doc.win ?? null,
...
async function evaluateStreaks(streakObjectives, player, matchId, smurfPuuid) {
  ...
  for (const obj of streakObjectives) {
    ...
    results.push({
      ...
      opponent_champion: recentMatches[0].opponentChampion || null,
      win: recentMatches[0].win ?? null,
      player_id: player._id.toString(),
      player_name: player.game_name,
      team_id: player.team_id,
      team_name: player.team_name,
...
async function fetchSoloQ() {
  const players = await Player.find({ puuid: { $exists: true, $ne: null }, connected_at: { $exists: true, $ne: null }, active: { $ne: false }, is_league: { $ne: true } });
