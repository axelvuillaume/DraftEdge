        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
          {connectedPlayers.length === 0 ? (
            <div className="col-span-full bg-slate-800/50 border border-slate-700/50 rounded-xl p-12 text-center">
              <p className="text-slate-500">No connected players. Add Riot IDs in the Team page first.</p>
            </div>
          ) : (
            connectedPlayers.map((player, i) => {
              const wins = player.current_wins || 0
              const losses = player.current_losses || 0
                ...
                >
                  ...
                  {total > 0 && (
                    <div className="flex items-center gap-3 text-xs">
                      <span className={winRate >= 50 ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>{winRate}%</span>
