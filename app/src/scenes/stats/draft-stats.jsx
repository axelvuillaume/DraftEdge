    ...
    <div className="h-[calc(100vh-200px)] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-5 overflow-y-auto">
      <div className="max-w-[1800px] w-full mx-auto flex flex-col gap-4">
        <div className="flex items-center justify-between gap-2">
          {pro ? (
            proFilters.team_name ? (
              <ProTeamRecord proFilters={proFilters} />
            ) : (
              <h2 className="text-lg font-bold text-white leading-none">Pro Games</h2>
            )
          ) : (
            <MyTeamRecord />
          )}
...
function ProTeamRecord({ proFilters }) {
  const [data, setData] = useState(null)

  const fetchData = async () => {
    try {
      const { ok, data, code } = await api.post("/pro-game/team-record", { ...proFilters })
      if (!ok) return toast.error(code || "Failed to fetch team record")
      setData(data)
    } catch (error) {
      toast.error(error.code || "Failed to fetch team record")
    }
  }
  ...
  return (
    <div className="flex items-baseline gap-2.5 min-w-0">
      <h2 className="text-lg font-bold text-white leading-none max-w-[280px] truncate">{proFilters.team_name}</h2>
      {data && (
        <>
...
function MyTeamRecord() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!ok) return toast.error(code || "Failed to fetch team stats")
      setData(data)
    } catch (error) {
      toast.error(error.code || "Failed to fetch team stats")
    }
  }
  ...
  return (
    <div className="flex items-baseline gap-2.5 min-w-0">
      <h2 className="text-lg font-bold text-white leading-none max-w-[280px] truncate">{user?.team_name || "My Team"}</h2>
      {data && (
        <>
          <span className="text-xs text-slate-400 whitespace-nowrap">
...
function FirstPickPanel({ pro, proFilters }) {
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-3 flex-shrink-0 min-w-[420px] flex flex-col">
      ...
      {!data ? (
        <div className="grid grid-cols-2 gap-3 flex-1 animate-pulse">
          {[0, 1].map(i => (
            <div key={i} className="rounded-xl border border-slate-700/30 bg-slate-700/10 p-2.5 flex flex-col gap-2">
              <div className="h-3 w-16 bg-slate-700/50 rounded" />
              {[...Array(6)].map((_, j) => (
                <div key={j} className="flex items-center gap-1.5">
                  <div className="w-6 h-6 rounded bg-slate-700/50 flex-shrink-0" />
                  <div className="h-2.5 flex-1 bg-slate-700/40 rounded" />
                </div>
              ))}
            </div>
          ))}
        </div>
      ) : !(data.rotations?.blue?.some(r => r?.length) || data.rotations?.red?.some(r => r?.length)) ? (
...
function BansPanel({ pro, proFilters }) {
  ...
  return (
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-3 flex-1 flex flex-col">
            ...
            <div key={i} className="rounded-xl border border-slate-700/30 bg-slate-700/10 p-2.5 flex flex-col gap-2">
              ...
              {[...Array(5)].map((_, j) => (
                <div key={j} className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-slate-700/50 flex-shrink-0" />
                  <div className="h-2.5 flex-1 bg-slate-700/40 rounded" />
                </div>
              ))}
            </div>
      ...
      ) : !(data.bans?.blue?.some(p => p?.length) || data.bans?.red?.some(p => p?.length)) ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-8 px-4">
          <Swords className="w-8 h-8 text-slate-600 mb-3" />
          <p className="text-sm text-slate-400 mb-1">No draft has been added to games</p>
          <p className="text-xs text-slate-500">
      ...
      {!data && (
        <div className="animate-pulse">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[...Array(4)].map((_, i) => (
              <div key={i} className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-3 flex flex-col gap-2">
                <div className="h-3 w-24 bg-slate-700/50 rounded mb-1" />
                {[...Array(5)].map((_, j) => (
                  <div key={j} className="flex items-center gap-2">
                    <div className="w-3.5 h-3.5 rounded bg-slate-700/50 flex-shrink-0" />
                    <div className="h-2 w-14 bg-slate-700/40 rounded" />
                    <div className="h-1 flex-1 bg-slate-700/40 rounded-full" />
      ...
      {(data || []).map(rota => (
        <div key={rota.rotation}>
          ...
          <div className="grid grid-cols-2 gap-3 items-start">
            {[rota.blue, rota.red].map((column, colIdx) => (
    ...
    <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-3 min-w-[320px]">
      ...
      {!data ? (
          ...
          {[...Array(4)].map((_, i) => (
            <div key={i} className="flex items-center gap-2 px-1.5 py-1.5">
              <div className="flex items-center -space-x-1.5">
                <div className="w-6 h-6 rounded bg-slate-700/50 relative z-10" />
                <div className="w-6 h-6 rounded bg-slate-700/50" />
              </div>
              <div className="h-2.5 flex-1 bg-slate-700/40 rounded" />
              <div className="h-3.5 w-8 bg-slate-700/50 rounded" />
            </div>
          ))}
        </div>
      ) : (
        <div className="flex flex-col gap-1">
          ...
          {data.length === 0 && <span className="text-slate-600 text-xs">—</span>}
        </div>
      )}
    </div>
  )
